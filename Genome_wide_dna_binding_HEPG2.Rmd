...
names(ehb_occupancy) == names(lhb_occupancy)
...
# How many genes are expressed but don't 
...
we are using the longest isoform, but this may not be the one that is active in this data. 
A better approach would be to do this on an isoform level.  
# Browser visualization shows that 1,933 genes expressed with 0 DBP have earlier start site
...
```{r}
# https://github.com/boulderrinnlab/CLASS_2021/blob/in_class/analysis/09_high_binding_properties/high_binding_properties.Rmd
library(countreg)

...
Let's look at this table in the RStudio viewer. 
```{r}
...
# You can see that there are a number of columns that we don't need and a number of empty columns. 
# It's seems mundane but starting here is the best way to make a "reproducible" sample sheet.
# You will see we have to take a lot of steps to clean all this up.
# Bottom line: the download to code to analysis is the way to reproducibility (this worked a year later :)
```

We will need to clean this up as well as append new data to this (such as the md5 checksum for each file). 
There are no md5 column in this table so we need to 'hack' into the ENCODE API to get this information.
...
# For example, this 'API code':
# field=read_count&field=md5sum&field=controlled_by&field=paired_end&field=paired_with&field=replicate&field=target

# this retrievies read_count, md5sum, controlled_by, paired_end, paired_with, replicate, and target
# the same as the buttons on the ENCODE portal browser but with more (e.g., md5sum)

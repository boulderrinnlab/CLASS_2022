...
The ENCODE API provides extensive documentation as to what data you can request and how to format that data to make your request. You can browse the possible requests in an interactive way using their interactive documentation: https://app.swaggerhub.com/apis-docs/encodeproject/api/basic_search
...
contstruct_query <- function(experiment_accession,
  ...
  #TODO why make the fields above and then change the names below?
  query <- paste(list(paste0("type=", type),
                      ...
                      #TODO explain what -paste0 is doing
...
samples <- samples %>%
  ...
  # TODO how does paired end identifier come after _read wouldn't it add _read to all values?
...
#TODO better understand this pivot wider
samplesheet <- samples %>%
...
samplesheet <- samplesheet %>%
  # cleaning up sample name that has spaces in it to replace with underscore
  mutate(condition = gsub(" ", "_", sample_name) %>% tolower()) %>%
  # making two new cols from one and keeping the original 
...
design <- samplesheet %>%
  ...
  mutate(sample = gsub(" ", "_", sample_name) %>% tolower(),
         ...
         # This pastes in the file path infrom of the fastq file name so making a driect path to fastq files
  ...
  # this is same code ran above to seperate out sample id into replicates and change to just the number

...
samples <- read.table("/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/samples.txt",
                      sep = "\t", skip = 1, header = T) %>%
...
#TODO somehow group by gets rid of the experiment_accession column
...
#TODO this runs where as the seperate chunks above do not :(

# TODO REFACTOR
```{r}
samples <- read.table("/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/fastq/samples.txt",
                      sep = "\t", skip = 1, header = T) %>%
  dplyr::rename(experiment_accession = Accession) %>%
  mutate(file_info = map(experiment_accession, ~ encode_file_info(.x))) %>%
  unnest(file_info) %>% 
...
write_lines(c("#!/bin/bash", rename_script$command), "/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/fastq/rename.sh")

# Here we use an R command to call bash and cd into that directory, then make the file executable, and then run it.

# TODO doesn't rename all the files :(
 system("cd /scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/fastq/; chmod u+x rename.sh; ./rename.sh")
```
...
md5 <- samples %>% 
  ungroup() %>%
  dplyr::select(md5sum, fastq) %>%
  unite(line, md5sum, fastq, sep = "  ")
write_lines(md5$line, "/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/fastq/md5.txt")
# Now let's run it.
 md5_results <- system("cd /scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/fastq; md5sum -c md5.txt")
 
# Nice they all look correct !
```
...
write_csv(samplesheet, "/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/samplesheet.csv")
```
...
design <- samplesheet %>%
  ...
  mutate(sample = gsub(" ", "_", sample_name) %>% tolower(),
         ...
         fastq_1 = paste0("/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/fastq/", fastq_1),
         fastq_2 = paste0("/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/fastq/", fastq_2)) %>%
...
#TODO need to remove replicate from design ....
...
write_csv(design, "/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/design.csv")

write_csv(samples, "/scratch/Shares/rinnclass/CLASS_2022/hepg2_RNAseq_test/samples.csv")

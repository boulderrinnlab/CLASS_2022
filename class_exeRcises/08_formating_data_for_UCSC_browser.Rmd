...
Now we are going to clean up and rename the files. We will use names and gsub to do this.
Note that when we use names we are telling the environment the name we want for this file.
Yet the file path is still the value. So names is kinda meta ...
```{R}
?names
?gsub

names(fl) <- gsub("/scratch/Shares/rinnclass/CLASS_2022/data/peaks/|_peaks.broadPeak", "", fl)
# note the pipe | is used as or statement (no spaces on either side of search term)
...
Now we are going to make a table of the peak files -- then we can print them out with header next
```{R}
?read.table
?lapply

# we are going to make an object peaks which stores each peak file -- when we read in with read.table
# we need to provide the column names -- these are the same as in the origninal .broadPeak file.

peaks <- lapply(fl[1:3], read.table, col.names = c("chr", "start", "end", "name", "score", "strand", 
                                                   "signal_value", "pval", "qval"))
summary(peaks)
names(peaks)
fl[1:3] # note that read.table is still using the file path to read in each file and names is a meta-header
```


Now let's filter out the non-canonical chromosomes
```{R}

# Make a object of the chromosomes we want to keep.
canonical_chr <- c(paste0("chr", 1:22), "chrM", "chrX", "chrY")

# now let's use lapply to filter across all the tables in "peaks" using a generic function(x)
# we pass this function 'x' to filter and it will only return enteries in peaks with chr
peaks <- lapply(peaks, function(x) x %>% filter(chr %in% canonical_chr))

```

Next we need a header to be compatible with UCSC 
We are using a broadPeak file which UCSC recognizes but we need to tell it which one it is in header
...
# note we can append variables printed out in R e.g., names(fl)[1:3]

new_filenames <- paste0("/Shares/rinn_class/data/CLASS_2022/data/ucsc/broadPeak/", names(fl)[1:3], ".broadPeak")

new_filenames
```

Cool, now we have an object "new_filenames" that has the file path we want to write out each file to.
...
I have hosted them on AWS here:

...
names(peaks) <- names(fl)[1:3]

# Filter out non-canon.
canonical_chr <- c(paste0("chr", 1:22), "chrM", "chrX", "chrY")
peaks <- lapply(peaks, function(x) x %>% filter(chr %in% canonical_chr))

# Write them out, with a track header
...
###########
sapply(peaks, nrow)
sapply(peaks_filt, nrow)
p1 <- peaks[[1]]



for (i in 1:length(peaks)) {
      peaks[[i]] <- peaks[[i]][seqnames(peaks[[i]]) %in% canonical_chr]
    }


# clean up file names to get info for header
...
Track Hub: http://genome.ucsc.edu/cgi-bin/hgTracks?db=hg38&hubUrl=https://bchm563classbucket.s3.amazonaws.com/trackhub/hub.txt

...
# now check out how to index into this using the handy
# Rstudio "clipboard", view object on right hand side clipboard icon appears.
# looks like we want:


#TODO how to export two columns into GTF so RTACKLAYER can reimport?
genes@elementMetadata@listData[["gene_id"]]
genes@elementMetadata@listData[["gene_name"]]
#TODO export mRNA and lncRNA annotations for RNAseq Analyse??
...
# cool let's write these out.
# /Shares/rinn_class/data/CLASS_2022/data/genome_features
...
Cleaning up samples sheet to just nuclear and cytoplasmic with more accessible names that the ENCODE accession number.
```{R}
...
# This way you can run DeSeq2 on other comparisons that could be useful!
```

We are almost ready, just a few more things to double check:

Triple check that the sample sheet and the counts are in right order. 
Reduce the sample and counts to just nuclear and cytoplasmic samples.
```{r}
...
#TODO write this out to save all this in the future??
...
write.csv2(res_df, "results/nuc_cyto_diff_expression_res_df.csv")
```

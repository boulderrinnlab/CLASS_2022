...
# same for changing insoluble_cytoplasmic_fraction condition to total
...
dds <- DESeqDataSetFromMatrix(countData = counts_filtered,
                              ...
                              design = ~ condition)
                              # perhaps most important is condition is a factor in samplesheet (look in env)
...
#TODO clean this up
# write_rds(rlog_counts_matrix, "test/rlog_counts_all.rds")
...
rlog_counts_matrix <- read_rds("path/rlog_counts_all.rds")

rlog_counts_matrix <- read_rds("/scratch/Shares/rinnclass/CLASS_2022/JR/CLASS_2022/class_exeRcises/analysis/19_rnaseq/test/rlog_counts_all.rds")
...
#TODO clean this up
# rlog_counts_matrix <- read_rds("test/rlog_counts_all.rds")
...
# TODO something goes wrong when intersecting to make counts_nuc_cyto
...
res_shrunken_df <- res_shrunken_df %>%
  mutate(subcellular_fraction = gsub("condition_|homo_sapiens_|_fraction_vs_homo_sapiens_hepg2", "", result_name))

table(res_shrunken_df$subcellular_fraction)
...
nuc_cyto_genes_to_plot <- res_shrunken_df %>%
  filter(gene_id %in% nuc_cyto_sig_genes$gene_id, subcellular_fraction %in% c("nuclear", "cytosolic"))

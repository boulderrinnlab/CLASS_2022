...
# TODO: note that right now, this pipeline run contains polyA and total RNA-seq
...
# TODO: work this in where you think it goes.
...
write_rds(samplesheet, "sample_sheets/final_samplesheet.rds")
...
write_rds(rlog_counts_matrix, "rlog_count_matrices/rlog_counts_all.rds")
...
# To keep the rownames, we can make sure it's a data.frame
...
write_rds(samplesheet_nuc_cyto, "sample_sheets/samplesheet_nuc_cyto.rds")
...
#TODO both rlog_counts read in have negative numbers
...
# full sample_sheet
samplesheet <- read_rds("sample_sheets/final_samplesheet.rds")

# nuc/cyto sample sheet
samplesheet_nuc_cyto <- read_rds("sample_sheets/samplesheet_nuc_cyto.rds")
...
# Let's subset the rlog_counts to just the nuclear and cyto samples
rlog_counts_matrix_nuc_cyto <- rlog_counts_matrix[,samplesheet_nuc_cyto$sample_name]
...
# Volcano plot
#TODO tring to index for colir
ggplot(res_df, aes(x = log2FoldChange, y = -log10(padj), color = -log10(res_df$padj) > 90)) + 
  geom_point()

# There is a limit floating precision and apparent for R it ends at 300-something
...
test <- read_csv("results/nuc_cyto_diff_expression_res_df.csv")

...
Let's run consensus_peaks -- so the function is loaded. But let's do this in debugging mode!!
Goto intersect_functions.R in util. then click on peak_list() line
Source on save 
then run create_consensus_peaks
...
hist(sapply(consensu_peaks, length))
sapply(consensus_peaks, length)
hist(width(consensus_peaks[[2]]))

...
cd scratch/Shares/rinnclass/'your_folder'/work
...
The design file will tell NF_CORE ChIPseq pipeline which sameples are which in a non-negotiable in format.
The design file will change depending on which NF_CORE Pipeline you are running

Let's make a design file:
...
It is super useful when running so many tasks to have an output file that updates at each step.
Also an error file that if something goes wrong explains exactly the line of code that broke.
...
Each line is a command ended by \ (make sure there are no extra spaces after \ -- a common bug)

...
# We can also just focus on lncRNA and mRNA promoters -- there are 30K other genes
# outside this that are mostly small RNAs pesudogenes .... let's not mix them in

all_promoters_gr <- rtracklayer::import("/Shares/rinn_class/data/CLASS_2022/data/genome_features/lncrna_mrna_promoters.gtf")
...
# An error will occur if there are not matching chromosomes. So if one chromosome wasn't bound it
...
# using base R here with 'as' a coericing function. as takes one dataset
...
# we want to flip so instead of 1:ncol we will do ncol:1 
...
# START POINT FOR MATRIX OF ALL DBPs BINDING PROFILE ACROSS PROMOTERS
How do the binding profiles over promoters compare across DBPs?
We can start to address this by 
...
# Can go from here to making individual plots
